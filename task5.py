...
def epochs_for_setting(setting: str, config: Dict[str, Any]) -> int:
    setting_epochs = config.get("task", {}).get("setting_epochs", {})
    value = setting_epochs.get(setting)
    if value is None:
        value = config["model"]["epochs"]
...
def run_single_setting(
    ...
    setting_epochs = epochs_for_setting(setting, config)
    total_steps = len(train_loader) * setting_epochs
    ...
    for epoch in range(setting_epochs):
        loss = train_one_epoch(
            ...
            device,
            config,
            desc=f"{experiment_id} epoch {epoch + 1}/{setting_epochs}",
        )
...
def generate_report_notes(metrics_df: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
    epoch_summary = ", ".join(
        f"{row['setting']}={int(row['epochs'])}" for _, row in metrics_df.iterrows()
    )
    notes = [
        (
            f"DistilBERT was fine-tuned on raw tweet text with max length {config['model']['max_length']}, "
            f"batch size {config['model']['batch_size']}, epochs by setting ({epoch_summary}), learning rate "
